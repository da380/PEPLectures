...
%DIF ADD diffs/lecture1_new.tex   Fri Sep 25 14:00:05 2026
...
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{.}\footnote{\DIFadd{I prefer the descriptive terms ``referential'' and ``spatial'' to describe the two means by which quantities can be 
described within a continuum. Some authors instead use ``Lagrangian'' and ``Eulerian'' for these concepts, though
...
\begin{enumerate}
    ...
    \label{eq:20}
    \end{equation}
    Again, \DIFaddbegin \DIFadd{the }\DIFaddend necessity of this condition can also be proven. Setting $\mathbf{Q}(t)=\mathbf{1}$, this identity reduces to
    \begin{equation}
...
gradient on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side are those associated with the given motion. Note that the Lagrangian density in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:24}) 
...
vanish for all smooth variations $\delta\bphi$ satisfying eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:26}). \DIFaddbegin \DIFadd{The first variation depends linearly on 
$\delta\bphi$, and it is often convenient to make this explicit by writing 
