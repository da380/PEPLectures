...
%DIF DEL lecture1.tex             Tue Aug 18 14:37:36 2026
%DIF ADD diffs/lecture1_new.tex   Fri Sep 25 16:34:46 2026
...
In this lecture, we derive the equations of motion for an elastic body. These equations provide a good\DIFaddbegin \DIFadd{, }\DIFaddend though not
complete\DIFaddbegin \DIFadd{, }\DIFaddend model for understanding seismic wave propagation and other related topics in geophysics. Most of you will
...
\begin{figure}[ht]
     \centering
     \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.7\textwidth]{figures/L1F1.png} 
%DIFDELCMD <      %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{figures/L1F1.pdf} 
     \DIFaddendFL \caption{A schematic illustration of the mapping from the reference body $M$ into the body's instantaneous position in physical space.}
     \label{fig:1}
 \end{figure}
...
we will not worry about that within this course. \DIFaddbegin \DIFadd{It is, however, worth knowing that this freedom can be put to real use: for example, the equations of motion for an 
aspherical earth model can be exactly rewritten as those for a spherically symmetric reference body, with the effects of 
boundary topography and lateral heterogeneity absorbed into modified material parameters. 
}\DIFaddend 
...
vector field\DIFdelbegin \footnote{\DIFdel{I prefer the descriptive terms ``referential'' and ``spatial'' to describe the two means by which quantities can be 
described within a continuum. Some authors instead use ``Lagrangian'' and ``Eulerian'' for these concepts, though
 both perspectives were introduced by Euler. }}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{.}\footnote{\DIFadd{I prefer the descriptive terms ``referential'' and ``spatial'' for the two ways in which quantities can be 
described within a continuum. Some authors instead use ``Lagrangian'' and ``Eulerian'' for these concepts, though
...
\label{eq:4}
\end{equation}
which is accurate to \DIFdelbegin \DIFdel{first-order }\DIFdelend \DIFaddbegin \DIFadd{first order }\DIFaddend in $\delta\mathbf{x}$. The deformation gradient is related to, but not identical with,
the concept of \textbf{strain}, \DIFdelbegin \DIFdel{with this }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend correspondence discussed further in the next lecture.
...
from the continuous differentiability of $\bphi$, implies that it is of one sign throughout the body \DIFaddbegin \DIFadd{(assumed connected)}\DIFaddend . We will assume that $J$ is always positive. This implies in particular that a right-handed set of 
...
\label{eq:8}
\end{equation}
where $\varrho(\mathbf{y},t)$ is the \textbf{spatial density} at the point $\mathbf{y}$ in $M_{t}$ at time $t$. During the deformation\DIFaddbegin \DIFadd{, }\DIFaddend matter is 
...
\label{eq:9}
\end{equation}
To simplify this expression we use the motion to transform the integration in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:9}) to the reference body
\begin{equation}
...
that eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:1}) holds at some time $t=t_{0}$\DIFaddbegin \DIFadd{, }\DIFaddend then at this time eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:12}) simplifies to
\begin{equation}
...
\textbf{hyperelastic} means that it has a well-defined potential energy. A consequence of this assumption\DIFdelbegin \DIFdel{-- that }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend you will establish 
in the first problem set\DIFdelbegin \DIFdel{-- }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend is that energy is conserved during the deformation of the body, and hence within this theory
we are discounting dissipative processes\DIFdelbegin \footnote{\DIFdel{The situation is analogous to particle mechanics restricted to the case of 
conservative forces, i.e., those forces that are derived from a potential energy.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{.}\footnote{\DIFadd{The situation is analogous to particle mechanics restricted to the case of 
conservative forces, i.e.\ those forces that are derived from a potential energy.}}
\DIFaddend Energy dissipation is relevant to seismology, but it 
...
Having accepted the form of the potential energy in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:16}), we are now in a position to obtain the equations of 
...
\begin{enumerate}
    ...
    and the potential energy is associated with deformations of the body away from this. Motions that do not alter its shape \DIFdelbegin \DIFdel{nor 
    }\DIFdelend \DIFaddbegin \DIFadd{or 
    }\DIFaddend volume should not, therefore, be associated with a change in potential energy. Within Euclidean space, motions that do 
    not change the relative lengths between points arise through a combination of rotations and translations.\DIFaddbegin \footnote{\DIFadd{Reflections also preserve lengths, but they are excluded by our assumption that $J>0$.}} \DIFaddend If $\bphi$ 
    ...
    \label{eq:20}
    \end{equation}
    Again, \DIFaddbegin \DIFadd{the }\DIFaddend necessity of this condition can also be proven. Setting \DIFdelbegin \DIFdel{$\mathbf{Q}(t)=\mathbf{1}$}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{Q}(t)=\mathbf{I}$}\DIFaddend , this identity reduces to
    \begin{equation}
    W[\mathbf{x},\bphi(\mathbf{x},t),\mathbf{F}(\mathbf{x},t)]=W[\mathbf{x},\mathbf{a}(t)+\bphi(\mathbf{x},t),\mathbf{F}(\mathbf{x},t)].
    \label{eq:21}
    \end{equation}
    For fixed $(\mathbf{x}, t)$, we can vary $\mathbf{a}(t)$ such that the second argument on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side takes any possible value.
     It follows that the potential energy density must be independent of its second argument. Given this result, it is useful \DIFdelbegin \DIFdel{to again 
     re-define }\DIFdelend \DIFaddbegin \DIFadd{once again 
     to redefine }\DIFaddend the potential energy density to depend only on $\mathbf{x}$ and $\mathbf{F}(\mathbf{x},t)$, and having done this 
     the condition in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:20}) reduces to
    \begin{equation}
...
\label{eq:23}
\end{equation}
where $W$ is subject to eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:22}). Within continuum mechanics this potential energy density is called the 
...
idea was developed by Clifford Truesdell and Walter Noll during the 1950s and 1960s \DIFaddbegin \DIFadd{(see Truesdell \& Noll 1965)}\DIFaddend , and plays a central role within all branches of theoretical continuum 
...
A Lagrangian density for a hyperelastic body is defined in the usual manner \DIFdelbegin \footnote{\DIFdel{For those with less experience with variational principles, 
the necessary ideas are developed here from first principles. But please ask in a supervision, or otherwise, if you would 
like any additional help with this material.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend as the difference between the kinetic and potential energy densities
\begin{equation}
...
gradient on the \DIFdelbegin \DIFdel{right hand }\DIFdelend \DIFaddbegin \DIFadd{right-hand }\DIFaddend side are those associated with the given motion.\DIFaddbegin \footnote{\DIFadd{For those with less experience with variational principles, 
the necessary ideas are developed here from first principles. But please ask in a supervision, or otherwise, if you would 
like any additional help with this material.}} \DIFaddend Note that the Lagrangian density in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:24}) 
...
\DIFdel{\delta\mathcal{S}=\lim_{h\rightarrow0}\frac{1}{h}[\mathcal{S}(\bphi+h~\delta\bphi)-\mathcal{S}(\bphi)]
%DIFDELCMD < \label{eq:27}%%%
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\delta\mathcal{S}=\lim_{h\rightarrow0}\frac{1}{h}[\mathcal{S}(\bphi+h\,\delta\bphi)-\mathcal{S}(\bphi)]
\label{eq:27}
}\end{equation}\DIFaddend 
vanish for all smooth variations $\delta\bphi$ satisfying eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:26}). \DIFaddbegin \DIFadd{The first variation depends linearly on $\delta\bphi$, and it is often convenient to write 
$\delta\mathcal{S}=\langle D\mathcal{S}(\bphi)|\delta\bphi\rangle$, where $D\mathcal{S}(\bphi)$ is called the 
}\textbf{\DIFadd{functional derivative}} \DIFadd{of the action at $\bphi$, and the angled brackets denote its linear action on a 
variation; we shall meet this notation again in later lectures. }\DIFaddend We readily find that this 
...
\end{align}
along with eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:26}) and the divergence theorem, we obtain
\begin{equation}
...
independently equal to zero\DIFdelbegin \footnote{\DIFdel{If this statement is not obvious, then you might wish to ask in a supervision. But for the 
purposes of this course it is sufficient to know how to apply this method.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{.}\footnote{\DIFadd{If this statement is not obvious, then you might wish to ask in a supervision. But for the 
purposes of this course it is sufficient to know how to apply this method.}} \DIFaddend Hamilton's principle, therefore, implies that 
the \textbf{\DIFdelbegin \DIFdel{Euler-Lagrange }\DIFdelend \DIFaddbegin \DIFadd{Euler--Lagrange }\DIFaddend equations}
\begin{equation}
...
For the Lagrangian density in eq.\DIFaddbegin \DIFadd{~}\DIFaddend (\ref{eq:24}) we readily find that
\begin{align}
...
\end{align}
The \textbf{first \DIFdelbegin \DIFdel{Piola-Kirchhoff }\DIFdelend \DIFaddbegin \DIFadd{Piola--Kirchhoff }\DIFaddend stress tensor} is defined by
\begin{equation}
T_{ij}(\mathbf{x},t)=\frac{\partial W}{\partial F_{ij}}[\mathbf{x},\mathbf{F}(\mathbf{x},t)],
\label{eq:36}
\end{equation}
so that the \DIFdelbegin \DIFdel{Euler-Lagrange }\DIFdelend \DIFaddbegin \DIFadd{Euler--Lagrange }\DIFaddend equations can be written
\begin{equation}
...
Indeed, what the first \DIFdelbegin \DIFdel{Piola-Kirchhoff }\DIFdelend \DIFaddbegin \DIFadd{Piola--Kirchhoff }\DIFaddend stress does is relate the orientation of a surface within the reference body 
...
we will see that within linearised theories about a stress-free equilibrium state the first \DIFdelbegin \DIFdel{Piola-Kirchhoff
}\DIFdelend \DIFaddbegin \DIFadd{Piola--Kirchhoff
}\DIFaddend stress and the Cauchy stress are equal, with this result accounting for the fact that their distinction 
