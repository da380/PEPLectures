...
%DIF ADD diffs/lecture9_new.tex   Fri Sep 25 12:46:06 2026
...
symmetric earth model with and without self-gravitation, as shown in Fig.~\ref{fig:2}, where attenuation has been 
...
$20\,\mathrm{km}$ depth, recorded at an epicentral angle of $90^{\circ}$, computed with (black) and without (red) 
self-gravitation. The top trace shows the full band of the calculation, from $0.3$ to $50\,\mathrm{mHz}$, and those below 
it the same seismogram band-pass filtered in successively lower frequency bands. Each trace is normalised by its 
maximum. The 
calculations neglect attenuation, so that the surface waves continue to circle the Earth undiminished over the six hours 
shown. The two calculations differ only in the lowest bands, and there the differences grow with time as the 
