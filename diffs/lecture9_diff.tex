...
%DIF ADD diffs/lecture9_new.tex   Fri Sep 25 16:34:56 2026
...
in $\mathbb{R}^{3}$, where $G$ is the universal gravitational constant, and $\varrho$ is the \DIFaddbegin \DIFadd{spatial }\DIFaddend density. The gravitational 
...
\DIFaddbegin \DIFadd{For the Earth, the equilibrium gravitational acceleration and the pressure that it produces are shown in Fig.~\ref{fig:1}. These 
...
\caption{\DIFaddFL{Gravitational acceleration (left) and pressure (right) as functions of radius in the earth model PREM, calculated 
from its density profile. The dashed lines mark the core--mantle boundary (CMB) and inner core boundary (ICB).}}
\label{fig:1}
\end{figure}

\DIFaddend It will be convenient to define the \textbf{referential gravitational potential} by
\DIFdelbegin \begin{displaymath}
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\delta\mathcal{V}_{g}=\frac{1}{4\pi G}\int_{M_{t}}\left(\frac{\partial^{2}\delta\phi}{\partial y_{i}\partial y_{i}}\right)\phi
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\left(\frac{\partial^{2}\delta\phi}{\partial y_{i}\partial y_{i}}\right)\phi=\frac{\partial}{\partial y_{i}}
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\delta\mathcal{V}_{g}=-\frac{1}{4\pi G}\int_{\mathbb{R}^{3}}\frac{\partial\phi}{\partial y_{i}}\frac{\partial\delta\phi}{\partial y_{i}}\dd^{3}\mathbf{y},
\label{eq:14}
}\end{equation}\DIFaddend 
where we have used the continuity of the potentials and their normal derivatives across $\partial M_{t}$, while the 
surface integral at infinity vanishes \DIFdelbegin \DIFdel{because $\phi\sim r^{-1}$ and $||\nabla\delta\phi||\sim r^{-2}$ as $r\rightarrow\infty$}\DIFdelend \DIFaddbegin \DIFadd{since, at large distances $r$, $\phi$ decays like $r^{-1}$ and $\|\nabla\delta\phi\|$ 
like $r^{-2}$}\DIFaddend . It follows that the increment to the gravitational binding energy can be written
\DIFdelbegin \begin{displaymath}
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\delta\mathcal{V}_{g}=\delta\left(-\frac{1}{8\pi G}\int_{\mathbb{R}^{3}}\frac{\partial\phi}{\partial y_{i}}
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\mathcal{V}_{g}=-\frac{1}{8\pi G}\int_{\mathbb{R}^{3}}\frac{\partial\phi}{\partial y_{i}}\frac{\partial\phi}{\partial y_{i}}\dd^{3}\mathbf{y}.
\label{eq:16}
}\end{equation}\DIFaddend 
...
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\frac{\partial\phi}{\partial y_{i}}\frac{\partial\phi}{\partial y_{i}}=\frac{\partial}{\partial y_{i}}
...
where we have used the identity \DIFdelbegin \DIFdel{$R_{kj}R_{kl}=\delta_{jl}$ }\DIFdelend \DIFaddbegin \DIFadd{$R_{ij}R_{kj}=\delta_{ik}$ }\DIFaddend for rotation matrices. \DIFdelbegin \DIFdel{In fact, if we differentiate this latter identity we find
}\begin{displaymath}
\DIFdel{\frac{\ddns R_{kj}}{\ddns t}R_{kl}+R_{kj}\frac{\ddns R_{kl}}{\ddns t}=0.
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{If we differentiate the
...
\textbf{Tisserand frame} which is defined such that the relative motion, $\overline{\bphi}$\DIFaddbegin \DIFadd{, }\DIFaddend carries no net linear
...
indifference, we then have \DIFdelbegin \DIFdel{$W(\mathbf{x},\mathbf{F})=W(\mathbf{x},\overline{\mathbf{F}})$ }\DIFdelend \DIFaddbegin \DIFadd{$W(\mathbf{x},t,\mathbf{F})=W(\mathbf{x},t,\overline{\mathbf{F}})$ }\DIFaddend as should be physically  
...
and hence will simplify \DIFdelbegin \DIFdel{notations }\DIFdelend \DIFaddbegin \DIFadd{notation }\DIFaddend by dropping overlines on terms linked to the relative motion. 
...
\DIFadd{\rho\epsilon_{ijk}\epsilon_{klm}\Omega_{j}\Omega_{l}u_{m}}&\DIFadd{\sim\overline{\rho}\Omega^{2}U, \label{eq:49} }\\
...
\DIFdelbegin \DIFdel{and using the Poisson equation }\DIFdelend \DIFaddbegin \DIFadd{Comparing eqs.~(\ref{eq:45}) and (\ref{eq:52}), the contribution of $u_{j}$ to $\gamma_{i}^{1}$ is
$-u_{j}\,\partial^{2}\zeta_{0}/\partial x_{i}\partial x_{j}$, whose magnitude is set by the trace }\DIFaddend $\nabla^{2}\zeta_{0}=4\pi G\rho$
\DIFdelbegin \DIFdel{along with eq.(45) }\DIFdelend \DIFaddbegin \DIFadd{given by Poisson's equation, and hence }\DIFaddend we arrive at the desired scaling estimate
\DIFdelbegin \begin{displaymath}
...
For such \DIFdelbegin \DIFdel{planetary scale }\DIFdelend \DIFaddbegin \DIFadd{planetary-scale }\DIFaddend deformations, it follows that gravitational forces are of the same order of magnitude as elastic forces, 
...
\DIFaddbegin \DIFadd{The conclusions of this scaling analysis can be checked directly by computing synthetic seismograms in a spherically
...
neglected so that the long-period waves persist over the six hours shown.}\footnote{\DIFadd{The calculations use the 
program }\texttt{\DIFadd{yspec}}\DIFadd{, which solves the equations of motion of this lecture in a spherically symmetric earth model by direct 
numerical integration in the frequency domain (Al-Attar \& Woodhouse 2008).}} \DIFadd{Over the full frequency band of the 
...
\caption{\DIFaddFL{Synthetic vertical-component acceleration seismograms in the earth model PREM for an earthquake of moment magnitude 8 at 
