...
as an initial comment, we note that each successive term within the series is divided by a factor of $1/(i\omega)$, and within the Fourier domain 
...
\begin{align}
\frac{\partial u_{k}}{\partial x_{l}}&=\sum_{n=0}^{\infty}\frac{1}{(i\omega)^{n}}\left(\frac{\partial a_{k}^{n}}{\partial x_{l}}
-i\omega p_{l}a_{k}^{n}\right)e^{-i\omega T}, \\
A_{ijkl}\frac{\partial u_{k}}{\partial x_{l}}&=\sum_{n=0}^{\infty}\frac{1}{(i\omega)^{n}}A_{ijkl}\left(\frac{\partial a_{k}^{n}}
{\partial x_{l}}-i\omega p_{l}a_{k}^{n}\right)e^{-i\omega T}.
\end{align}
...
{(i\omega)^{n}}\left[\frac{\partial}{\partial x_{j}}\left(A_{ijkl}\frac{\partial a_{k}^{n}}{\partial x_{l}}\right)
...
The above equation must hold for all frequencies, and this can only be true if the coefficient of each power of $i\omega$ is separately equal to zero. From the coefficients of the highest-order term $(i\omega)^{2}$ we obtain
\begin{equation}
