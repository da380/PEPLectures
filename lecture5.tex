...
For definiteness, let $p_{i}^{0}$ be the slowness vector of a chosen plane wave in the homogeneous part of the body, and 
...
the length scale of the incident wave. The colours show the magnitude of the displacement, the arrows show the particle motion, and the dashed line marks the extent of the heterogeneity; the colour scale is saturated so as to show the weak scattered field. The box is about 13 wavelengths of the incident wave wide and 6 high. We see that the scattered wavefield comprises both P-waves and S-waves.}
\label{fig:1}
\end{figure}
...
(not to be confused with the Lam\'{e} parameter) denote the \textbf{dominant wavelength for the plane wave}, and denote by $L$ the \textbf{smallest length scale over which 
the material parameters vary to an appreciable extent}. If their ratio satisfies
\begin{equation}
\frac{L}{\lambda}\approx1,
\label{eq:2}
\end{equation}
then we expect the incident wave to be \textbf{scattered by the heterogeneity} (see Fig.~\ref{fig:1}). There is not sufficient time in this course to 
...
wavefront as it propagates through the heterogeneity, along with associated amplitude variations due to focusing effects (see Fig.~\ref{fig:2}). This 
...
\label{eq:11}
\end{equation}
where we do not include a body force associated with a seismic source. Within the temporal Fourier domain this equation becomes
\begin{equation}
...
To do this, it will be useful to define the \textbf{local Christoffel matrix} through
\begin{equation}
...
determined uniquely up to its sign. In an isotropic medium the two S-wave sheets of the slowness surface coincide, and so this assumption 
...
determined uniquely by eq.~(\ref{eq:22}). Under this non-degeneracy assumption we can therefore always write the eikonal equation unambiguously in the form
\begin{equation}
...
\label{eq:27}
\end{equation}
which is defined on a six-dimensional \textbf{phase space} comprising points $(\mathbf{x}, \mathbf{p})$, and allows us 
...
\label{eq:28}
\end{equation}
Eq.~(\ref{eq:28}) states that the desired solution, $T$, of the eikonal equation corresponds to a \textbf{three-dimensional surface} $S=\{(\mathbf{x},\nabla T(\mathbf{x}))\}$ lying within 
 the level set $H_{k}(\mathbf{x},\mathbf{p})=\frac{1}{2}$ of the ray Hamiltonian. This 
...
In this manner, the initial data for the problem specifies a \textbf{two-dimensional surface $S^{0}$ lying within this level set}, and to 
solve the eikonal equation we must construct
 a three-dimensional surface $S$ within the level set that contains $S^{0}$.

Let $(\mathbf{x}^{0},\mathbf{p}^{0})$ be a point on the initial surface $S^{0}$, and consider the solution of 
...
turn each point $(\mathbf{x}^{0},\mathbf{p}^{0})$ of the initial surface $S^{0}$, and integrate Hamilton's equations to produce a 
curve on which the ray Hamiltonian is equal to one half. The union of all these curves defines a smooth three-dimensional 
surface $S$ lying within the level set $H_{k}=\frac{1}{2}$, and this gives us the desired solution of the eikonal equation in geometric form. 
...
Having constructed the surface $S$, how do we recover the travel time $T$? We first make use of eqs.~(\ref{eq:7}) and (\ref{eq:30}) to obtain
\begin{equation}
...
\label{eq:35}
\end{equation}
for all $\gamma>0$, and so Euler's theorem for homogeneous functions can be applied.\footnote{
    ...
    and this is the only case in which you might be asked to reproduce this derivation.} It implies that
\begin{equation}
...
large but finite amplitudes (the name comes from the Greek for ``burning''), and more 
...
Though within this lecture we have focused on the problem of an initially plane wave entering into a heterogeneous region, it should
...
\begin{enumerate}
    ...
    \item[(ii)] How to obtain the eikonal equation and transport equations by substituting the ray expansion into the equations of motion. The full derivation given in the lecture is too involved to be set in an exam, but you may be asked to apply these methods within simpler situations.
