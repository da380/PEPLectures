...
# drawn at its printed width (\textwidth) so that the fonts appear at their nominal size; kernels in 1e-4 / km
fig, axes = plt.subplots(2, 4, figsize=(6.5, 5.0), sharey=False, constrained_layout=True)
SCALE = 1e3 * 1e4
for ax, (l, n) in zip(axes[0], [(2, 0), (2, 1), (10, 0), (40, 0)]):
    ...
    ax.plot(Kmu * mu / om * SCALE, r, "k", lw=1.3, label=r"$\mu$")
    ax.plot(Krho * rho / om * SCALE, r, "tab:red", lw=1.3, label=r"$\rho$")
...
for ax, (l, n) in zip(axes[1], [(2, 0), (2, 1), (10, 0), (0, 0)]):
    ...
    ax.plot(Kk * kap / om * SCALE, rc, "tab:blue", lw=1.3, label=r"$\kappa$")
    ax.plot(Km * mu / om * SCALE, rc, "k", lw=1.3, label=r"$\mu$")
    ax.plot(Kr * rho / om * SCALE, rc, "tab:red", lw=1.3, label=r"$\rho$")
