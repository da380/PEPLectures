...
Having obtained the equations of motion for a self-gravitating earth model, we begin our study of the Earth's free oscillations or normal modes (the terms are synonymous). The starting point is developing a concise operator notation that closely resembles what you know from quantum mechanics. We analyse the associated eigenvalue problem in a non-rotating earth model, with the mathematics again being near identical to that in quantum physics. In particular, the forced response of such an earth model is obtained as an eigenfunction expansion, with the frequency-domain response having resonances at each of the eigenfrequencies. It is due to this fact that the observations of the eigenfrequencies can be made from sufficiently long time-series, with these data providing important constraints on Earth structure. Indeed, such frequencies form the principal constraint on the Earth's density. To conclude, we outline the steps required to account for rotation, this involving solution of two non-linear eigenvalue problems.
...
\begin{equation}
\int_{M}\left[\rho\left(\frac{\partial^{2}u_{i}}{\partial t^{2}}+2\epsilon_{ijk}\Omega_{j}\frac{\partial u_{k}}{\partial t}+\epsilon_{ijk}\epsilon_{klm}\Omega_{j}\Omega_{l}u_{m}\right)-\frac{\partial}{\partial x_{j}}(A_{ijkl}\frac{\partial u_{k}}{\partial x_{l}}-\overline{S}_{ij})-\rho\gamma_{i}^{1}\right]w_{i}^{*}d^{3}x=0.
\end{equation}
...
\begin{equation}
\int_{M}\left[\rho\left(\frac{\partial^{2}u_{i}}{\partial t^{2}}+2\epsilon_{ijk}\Omega_{j}\frac{\partial u_{k}}{\partial t}+\epsilon_{ijk}\epsilon_{klm}\Omega_{j}\Omega_{l}u_{m}-\gamma_{i}^{1}\right)w_{i}^{*}+(A_{ijkl}\frac{\partial u_{k}}{\partial x_{l}}-\overline{S}_{ij})\frac{\partial w_{i}^{*}}{\partial x_{j}}\right]d^{3}x=0.
\end{equation}
...
\begin{equation}
\langle \mathbf{w}|P|\mathbf{u}\rangle=\int_{M}\rho w_{i}^{*}u_{i}d^{3}x,
\end{equation}
...
\begin{equation}
\langle \mathbf{w}|W|\mathbf{u}\rangle=\int_{M}2\rho\epsilon_{ijk}w_{i}^{*}\Omega_{j}u_{k}d^{3}x
\end{equation}
...
\begin{equation}
\langle \mathbf{w}|H|\mathbf{u}\rangle=\int_{M}\left(\rho\epsilon_{ijk}\epsilon_{klm}w_{i}^{*}\Omega_{j}\Omega_{l}u_{m}+A_{ijkl}\frac{\partial w_{i}^{*}}{\partial x_{j}}\frac{\partial u_{k}}{\partial x_{l}}-\rho w_{i}^{*}\gamma_{i}^{1}\right)d^{3}x,
\end{equation}
and write the force term as
\begin{equation}
\langle \mathbf{w}|\mathbf{f}\rangle=\int_{M}\overline{S}_{ij}\frac{\partial w_{i}^{*}}{\partial x_{j}}d^{3}x.
\end{equation}
...
\begin{equation}
\langle \mathbf{w}|W|\mathbf{u}\rangle=\int_{M}2\rho\epsilon_{ijk}w_{i}^{*}\Omega_{j}u_{k}d^{3}x = -\left(\int_{M}2\rho\epsilon_{ijk}u_{i}^{*}\Omega_{j}w_{k}d^{3}x\right)^{*} = -\langle \mathbf{u}|W|\mathbf{w}\rangle^{*},
\end{equation}
...
\begin{equation}
\int_{M}\rho\epsilon_{ijk}\epsilon_{klm}w_{i}^{*}\Omega_{j}\Omega_{l}u_{m}d^{3}x=\left(\int_{M}\rho\epsilon_{ijk}\epsilon_{klm}w_{i}\Omega_{j}\Omega_{l}u_{m}^{*}d^{3}x\right)^{*} = \left(\int_{M}\rho\epsilon_{ijk}\epsilon_{klm}u_{i}^{*}\Omega_{j}\Omega_{l}w_{m}d^{3}x\right)^{*},
\end{equation}
...
\begin{equation}
\int\rho w_{i}^{*}\gamma_{i}^{1}(\mathbf{u})d^{3}x=\int\rho\gamma_{i}^{1}(\mathbf{w})^{*}u_{i}d^{3}x.
\end{equation}
...
The following additional properties of these eigenfunctions and eigenfrequencies will be stated without proof\footnote{The validity of these results in solid earth models can be established rigorously based on the theory of elliptic partial differential operators. Their proof, and indeed general validity, within models containing in-viscid fluid regions like the outer core remains an open problem.}:
\begin{enumerate}
    \item Each squared-eigenfrequency is associated with a finite-dimensional eigenspace. This means that while there may be more than one linearly independent eigenfunction associated with a squared-eigenfrequency, there is always a finite-dimensional basis for this linear subspace. Eigenfunctions within the same eigenspace need not be orthogonal, but we can always select a basis such that they are.
    \item There is a countable infinity of squared-eigenfrequencies. Their values are bounded from below, and accumulate only at infinity. The latter point means that in a finite-interval there can only be a finite number of squared-eigenfrequencies.
    \item The eigenfunctions form a complete orthonormal basis for vector fields in the earth model. This means that an arbitrary vector field can be expressed as a convergent sum of the eigenfunctions, with the necessary expansion coefficients readily determined due to their orthogonality.
\end{enumerate}
To proceed, we let $k=0,...,\infty$ label the different squared eigenfrequencies, $\omega_{k}^{2}$. For each k, there is then a finite-dimensional eigenspace, and we use a second index m to label the elements of its basis. Note that the range of m can vary with k, but we leave this implicit within the notation. The mth eigenfunction belonging to the kth eigenspace will be written $|km\rangle$ in a manner familiar from quantum mechanics. These eigenfunctions can be orthonormalised relative to the kinetic energy form, this meaning that
\begin{equation}
\langle km|P|k'm'\rangle=\delta_{kk'}\delta_{mm'},
\end{equation}
for all possible indices. Using this notation, we can then express the completeness of the eigenfunctions by noting that a displacement field $\mathbf{u}$ can be expanded as
\begin{equation}
...
Looking at eq. (34), it is notable that there are singularities at $\pm\omega_{k}$ for each k. Supposing for the moment that the squared-eigenfrequencies are all positive, these singularities lie along the real line. Physically, we are seeing resonances in the frequency-domain response when the system is driven at one of its eigenfrequencies. From an actual seismic record we could take its Fourier transform, look for associated resonance peaks, and hence measure some of the eigenfrequencies of the Earth. Note that these frequencies are determined solely by Earth structure, and hence their measurement provides a possible means to avoid the source-structure trade-off that is ever present in seismology.
...
\begin{equation}
\langle \mathbf{u}|H|\mathbf{u}\rangle=\int_{M}A_{ijkl}\frac{\partial u_{i}^{*}}{\partial x_{j}}\frac{\partial u_{k}}{\partial x_{l}}d^{3}x,
\end{equation}
...
\begin{equation}
\langle \mathbf{u}|H|\mathbf{u}\rangle=\frac{1}{4}\int_{M}A_{ijkl}\left(\frac{\partial u_{i}}{\partial x_{j}}+\frac{\partial u_{j}}{\partial x_{i}}\right)^{*}\left(\frac{\partial u_{k}}{\partial x_{l}}+\frac{\partial u_{l}}{\partial x_{k}}\right)d^{3}x,
\end{equation}
...
\begin{equation}
\langle \mathbf{u}|H|\mathbf{u}\rangle=\frac{1}{4}\int_{M}A_{ijkl}\left(\frac{\partial u_{i}}{\partial x_{j}}+\frac{\partial u_{j}}{\partial x_{i}}\right)^{*}\left(\frac{\partial u_{k}}{\partial x_{l}}+\frac{\partial u_{l}}{\partial x_{k}}\right)d^{3}x>0,
\end{equation}
...
Once gravitation is added into the problem it remains true that there is a six-dimensional eigenspace at zero-frequency associated with rigid body motions. The issue of stability of the remaining modes is, however, complicated by the fact that the potential energy has a gravitational contribution. While it is possible to examine this problem quantitatively, it requires some fairly advanced techniques from functional analysis. The physically important point, however, is that self-gravitation is an inherently destabilising force. Consider, for example, a spherically symmetric and self-gravitating planet at equilibrium. If we could compress this planet uniformly, then there would be an increase in elastic potential energy. At the same time, the mass of the planet has been brought into a smaller volume, and hence the gravitational binding energy must decrease\footnote{Recall that this is a negative quantity, and hence it has increased in magnitude.}. The net effect on the potential energy depends on the relative magnitude of these terms. If the elastic constants are sufficiently large, then the elastic term will dominate and the planet will be stable with respect to such a perturbation. But if gravity is sufficiently strong then the planet will keep pulling itself inwards until non-linear effects take over.

\subsection*{Eigenfunctions and eigenvalues in a rotating earth model}
We conclude the lecture by discussing modifications to the theory required to account for rotation. Working again in the frequency-domain, the equations of motion are
\begin{equation}
-\omega^{2}\langle \mathbf{w}|P|\tilde{\mathbf{u}}\rangle+i\omega\langle \mathbf{w}|W|\tilde{\mathbf{u}}\rangle+\langle \mathbf{w}|H|\tilde{\mathbf{u}}\rangle=\langle \mathbf{w}|\tilde{\mathbf{f}}\rangle,
\end{equation}
which must hold for an arbitrary test function $\mathbf{w}$. Suppose that, for fixed $\omega$, there exists a non-trivial solution, $\mathbf{s}$, of the equation
\begin{equation}
-\omega^{2}\langle \mathbf{w}|P|\mathbf{s}\rangle+i\omega\langle \mathbf{w}|W|\mathbf{s}\rangle+\langle \mathbf{w}|H|\mathbf{s}\rangle=0,
\end{equation}
for all $\mathbf{w}$. If this holds, then from any solution, $\tilde{\mathbf{u}}$, of eq.(45) we could obtain another solution $\tilde{\mathbf{u}}+\lambda \mathbf{s}$ with $\lambda$ an arbitrary scalar. Unique solutions of eq.(45) can, therefore, only exist if eq.(46) has no non-trivial solutions.

Next, suppose that there exists a non-trivial solution, $\overline{\mathbf{s}}$, of the equation
\begin{equation}
-\omega^{2}\langle\overline{\mathbf{s}}|P|\mathbf{w}\rangle+i\omega\langle\overline{\mathbf{s}}|W|\mathbf{w}\rangle+\langle\overline{\mathbf{s}}|H|\mathbf{w}\rangle=0,
\end{equation}
for all $\mathbf{w}$. If we take such an $\overline{\mathbf{s}}$ as the test function in eq. (45) we obtain
\begin{equation}
\langle\overline{\mathbf{s}}|\mathbf{f}\rangle=0.
\end{equation}
It follows that if eq.(47) has non-trivial solutions, then the frequency-domain problem in eq.(45) admits solutions only if eq.(48) holds. In this manner we have arrived at two necessary conditions for eq.(45) to be solvable in a unique way. It can be shown that these two conditions are also sufficient. For clarity we restate the main conclusions:
\begin{enumerate}
    \item The frequency-domain problem in eq.(45) has solutions if and only if eq. (48) holds for any non-trivial solutions of eq.(47);
    \item The solutions of eq. (45) are uniquely defined if and only if eq.(46) admits no non-trivial solutions.
\end{enumerate}
Writing eq. (46) again for convenience
\begin{equation}
-\omega^{2}\langle \mathbf{w}|P|\mathbf{s}\rangle+i\omega\langle \mathbf{w}|W|\mathbf{s}\rangle+\langle \mathbf{w}|H|\mathbf{s}\rangle=0,
\end{equation}
we note that this is a type of eigenvalue problem, with $\omega$ the eigenvalue parameter and $\mathbf{s}$ the eigenfunction. Indeed, if we set $\Omega_{i}$ equal to zero the problem reduces to eq. (19), which is an eigenvalue problem of standard form with $\omega^{2}$ the eigenvalue parameter. Because in eq.(46) the eigenvalue parameter appears non-linearly within the equation, we say that it is a non-linear eigenvalue problem.

Turning to eq.(47), we use symmetries of the different terms to to re-write it as
\begin{equation}
-\overline{\omega}^{2}\langle \mathbf{w}|P|\overline{\mathbf{s}}\rangle+i\overline{\omega}\langle \mathbf{w}|W|\overline{\mathbf{s}}\rangle+\langle \mathbf{w}|H|\overline{\mathbf{s}}\rangle=0,
\end{equation}
where we have defined $\overline{\omega}=\omega^{*}$. This again is a non-linear eigenvalue problem with eigenvalue parameter $\overline{\omega}$ and eigenfunction $\overline{\mathbf{s}}$. In fact, the eigenvalue problems in eq.(46) and (50) have the same form. It can be shown that non-trivial solutions to eq. (46) and eq. (50) exist only for a countably infinite number of frequencies. This is then the same as in the non-rotating case, though note that we gave no proof for that result either. Similarly, the eigenspaces are all finite-dimensional, and the eigenfunctions from either problem form a complete basis. What is different, however, is that the eigenfunctions belonging to distinct eigenspaces are no longer orthogonal relative to the kinetic energy form. This fact complicates significantly the derivation of a solution to the forced problem as an eigenfunction expansion, and we will not enter into the details. Such a solution is, however, not widely used, with the mode coupling approach discussed in the next lecture being more useful. Nonetheless, the basic form of the eigenfunction expansion is similar, with the eigenfunction, $\mathbf{s}_{k}$ with eigenfrequency $\omega_{k}$ contributing a term proportional to
\begin{equation}
\frac{1}{\omega-\omega_{k}}
\end{equation}
within the eigenfunction expansion. There are still, therefore, resonances associated with each eigenfrequency that can be measured from sufficiently long seismograms.
...
\begin{itemize}
    ...
    \item[(vi)] How the non-linear eigenvalue problems for a rotating earth model arises, along with its qualitative properties. This include the role of rotation within stability of the earth model as discussed in the second problem set.
\end{itemize}
