...
\end{equation}
where $[\cdot]_{-}^{+}$ denotes the jump in a quantity in the direction of outward normal.
...
\begin{equation}
\delta\mathcal{V}_{g}=\frac{1}{4\pi G}\int_{M_{t}}\left(\frac{\partial^{2}\delta\phi}{\partial x_{i}\partial x_{i}}\right)\phi
...
Using the Poisson equation for $\phi$ we can write the above expression in the form
\begin{equation}
...
\end{equation}
where $\mathbf{R}(t)$ is a rotation matrix. Here $\mathbf{\Phi}(t)$ denotes  translation of the reference frame relative 
...
approach is to take
\begin{equation}
\frac{\ddns \mathbf{\Phi}}{\ddns t}=\mathbf{0}, \quad \frac{\ddns \mathbf{\Omega}}{\ddns t}=\mathbf{0},
\end{equation}
...
\textbf{Tisserand frame} which is defined such that the relative motion, $\overline{\bphi}$ carries  no net linear
 nor angular momentum\footnote{See ``On the elastodynamics of rotating planets'' by Maitra \& Al-Attar (2024) for details.}.
...
undergoing a steadily rotation relative to inertial space. We consider a disturbance to the body
caused by a stress glut, and  hence the strain energy to be
\begin{equation}
...
must be a balance between the centrifugal, gravitational forces on the body and a resulting equilibrium stress; the 
...
differential equations, but  \textbf{integro partial differential equations}, with 
