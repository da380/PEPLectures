...
\begin{figure}[ht]
     \centering
     \includegraphics[width=0.7\textwidth]{figures/L1F1.pdf} 
...
we will not worry about that within this course. It is, however, worth knowing that this freedom can be put to real use: for example, the equations of motion for an 
...
vector field.\footnote{I prefer the descriptive terms ``referential'' and ``spatial'' for the two ways in which quantities can be 
...
the concept of \textbf{strain}, a correspondence discussed further in the next lecture.
...
from the continuous differentiability of $\bphi$, implies that it is of one sign throughout the body (assumed connected). We will assume that $J$ is always positive. This implies in particular that a right-handed set of 
...
\textbf{hyperelastic} means that it has a well-defined potential energy. A consequence of this assumption, which you will establish 
...
\begin{enumerate}
    ...
    and the potential energy is associated with deformations of the body away from this. Motions that do not alter its shape or 
    volume should not, therefore, be associated with a change in potential energy. Within Euclidean space, motions that do 
    not change the relative lengths between points arise through a combination of rotations and translations.\footnote{Reflections also preserve lengths, but they are excluded by our assumption that $J>0$.} If $\bphi$ 
    ...
    \label{eq:20}
    \end{equation}
    Again, the necessity of this condition can also be proven. Setting $\mathbf{Q}(t)=\mathbf{I}$, this identity reduces to
    \begin{equation}
    ...
    For fixed $(\mathbf{x}, t)$, we can vary $\mathbf{a}(t)$ such that the second argument on the right-hand side takes any possible value.
     It follows that the potential energy density must be independent of its second argument. Given this result, it is useful once again 
     to redefine the potential energy density to depend only on $\mathbf{x}$ and $\mathbf{F}(\mathbf{x},t)$, and having done this 
...
idea was developed by Clifford Truesdell and Walter Noll during the 1950s and 1960s (see Truesdell \& Noll 1965), and plays a central role within all branches of theoretical continuum 
...
gradient on the right-hand side are those associated with the given motion.\footnote{For those with less experience with variational principles, 
the necessary ideas are developed here from first principles. But please ask in a supervision, or otherwise, if you would 
like any additional help with this material.} Note that the Lagrangian density in eq.~(\ref{eq:24}) 
...
\label{eq:27}
\end{equation}
vanish for all smooth variations $\delta\bphi$ satisfying eq.~(\ref{eq:26}). The first variation depends linearly on $\delta\bphi$, and it is often convenient to write 
$\delta\mathcal{S}=\langle D\mathcal{S}(\bphi)|\delta\bphi\rangle$, where $D\mathcal{S}(\bphi)$ is called the 
\textbf{functional derivative} of the action at $\bphi$, and the angled brackets denote its linear action on a 
variation; we shall meet this notation again in later lectures. We readily find that this 
