...
discuss \textbf{finite elasticity}, this meaning that there is no assumption that the deformation has to remain  small
in an appropriate sense. Within later applications
it will actually be sufficient to consider linearised motions about an equilibrium state, but by starting with the
exact theory it is easier to appreciate the underlying physics.

\section*{Basic definitions}
We start by considering the kinematics of a solid body. At an instant in time, $t$, the body occupies a subset $M_{t}$ 
of physical space, $\mathbb{R}^{3}$. A useful way to describe the motion is through the introduction of a reference body, 
$M$. Each point $\mathbf{x}$ in the reference body corresponds to a unique particle. At time $t$, the particle $\mathbf{x}$ 
sits at a point in physical space that we denote by $\bphi(\mathbf{x},t)$. In this manner we see that the body's motion is 
completely described by this time-dependent mapping, $\bphi(\cdot,t)$, from the reference body into physical space. We will 
call $\bphi$ the \textbf{motion} of the body, this being the field within this classical field theory. We will not consider motions 
for which the body is torn or fractured. As a result, we suppose that $\bphi$ is continuously differentiable, and that at 
each fixed time, $t$, it establishes a one-to-one correspondence between $M$ and $M_{t}$. These ideas are summarised 
graphically within Fig.1.
...
\label{eq:1}
\end{equation}
for all $\mathbf{x}$. It can, however, sometimes be useful to select a different reference body for both theoretical 
and computational reasons.

\section*{Velocity, deformation gradient, and Jacobian}
If we fix attention to a particle $\mathbf{x}$, then the function $t\mapsto\bphi(\mathbf{x},t)$ describes its 
...
\label{eq:2}
\end{equation}
Note that this is the velocity of a fixed particle, and not at a fixed spatial point. This differs from what is commonly 
...
\label{eq:5}
\end{equation}
Invertibility of the deformation gradient implies that $J$ is never equal to zero, while continuity of $\bphi$ implies that 
it is always of one sign. We will assume that $J$ is always positive. This implies in particular that a right-handed set of 
...
\begin{equation}
\int_{U_{t}}\rho(\mathbf{y},t)\dd^{3}\mathbf{y},
\label{eq:8}
\end{equation}
where $\rho(\mathbf{y},t)$ is the density at the point $\mathbf{y}$ in $M_{t}$ and time $t$. During the deformation matter is not 
created nor destroyed. This means that the mass of this arbitrary sub-body is constant, and hence we must have
\begin{equation}
\frac{d}{dt}\int_{U_{t}}\rho(\mathbf{y},t)\dd^{3}\mathbf{y}=0.
\label{eq:9}
\end{equation}
To simplify this expression we use the motion to transform the integration in eq.(\ref{eq:9}) to the reference body
\begin{equation}
\frac{d}{dt}\int_{U}J(\mathbf{x},t)\rho[\bphi(\mathbf{x},t),t]\dd^{3}\mathbf{x}=0.
\label{eq:10}
\end{equation}
...
$U$ is an arbitrary subset of $M$, the equality can hold if and only if
\begin{equation}
\frac{\partial}{\partial t}\{J(\mathbf{x},t)\rho[\bphi(\mathbf{x},t),t]\}=0.
\label{eq:11}
\end{equation}
It follows we can introduce a time-independent \textbf{referential density} $\rho_{0}$ such that
\begin{equation}
\rho_{0}(\mathbf{x})=J(\mathbf{x},t)\rho[\bphi(\mathbf{x},t),t]
\label{eq:12}
\end{equation}
...
\begin{equation}
\mathcal{T}(t)=\frac{1}{2}\int_{M}\rho_{0}(\mathbf{x})||\mathbf{v}(\mathbf{x},t)||^{2}\dd^{3}\mathbf{x}.
\label{eq:15}
\end{equation}
...
\label{eq:16}
\end{equation}
where $W$ is a potential energy density measured per unit volume within the reference body. This is not the most general form 
...
\begin{enumerate}
    ...
    where $\mathbf{Q}(t)$ is a rotation matrix. Because these motions differ only by a superimposed rigid body motion, they 
...
\textbf{strain energy function}, and this what we will say from now on. The invariance of the potential energy under (i) time-translation, 
...
idea was introduced by Clifford Truesdell in the 1950s, and plays a central role within all branches of theoretical continuum 
...
the necessary ideas are developed here from first principles. But please ask either in the supervisions or otherwise if you would 
like any additional help with this material.} as the difference between the kinetic and potential energy densities
\begin{equation}
\mathcal{L}(\mathbf{x},t,\bphi,\mathbf{v},\mathbf{F})=\frac{1}{2}\rho_{0}(\mathbf{x})||\mathbf{v}||^{2}-W(\mathbf{x},\mathbf{F})
\label{eq:24}
\end{equation}
...
shows no explicit dependence on $t$ nor $\bphi$, but we shall 
...
\begin{equation}
\delta\bphi(\mathbf{x},0)=\delta\bphi(\mathbf{x},T)=0.
\label{eq:26}
\end{equation}
...
\label{eq:27}
\end{equation}
be equal to zero for all smooth variations $\delta\bphi$ satisfying eq.(\ref{eq:26}). We readily find that this 
...
\label{eq:32}
\end{equation}
where $\hat{\mathbf{n}}$ denotes the outward unit normal vector to $\partial M$, and $dS$ is the surface element. 
As $\delta\bphi$ is arbitrary, this first variation can vanish only if both the volumetric term and boundary term are 
...
\label{eq:38}
\end{equation}
The vector $\mathbf{t}=\mathbf{T}\hat{\mathbf{n}}$ occurring in the boundary conditions is known as the traction. Within the 
first problem set you will see that it represents a surface force measured per unit area within the reference body. 
Indeed, what the first Piola-Kirchhoff stress does is relate the orientation of a surface within the reference body 
to the force per unit area acting on the instantaneous image of that surface within physical space. 
...
\begin{enumerate}
    ...
    \item[(iii)] State expressions for the kinetic and potential energies of a simple hyperelastic body, and to 
