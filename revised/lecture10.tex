...
stress associated with mantle convection scales like $\eta V/L$ with $\eta\sim10^{21}\,\mathrm{Pa\,s}$ a typical viscosity, $V\sim10^{-8}\,\mathrm{m\,s^{-1}}$ a 
...
around $\eta V/L\sim10^{7}\,\mathrm{Pa}$. Pressures in the mantle are, however, known to be of order $10^{10}\,\mathrm{Pa}$,\footnote{Values for the 
...
\label{eq:37}
\end{equation}
whose derivation is left as an exercise in the second problem set. Using eq.~(\ref{eq:35}), we see that Poisson's equation is transformed into
\begin{equation}
...
\section*{Rapidly rotating bodies}
The theory above is restricted to slow rotation, but it is worth describing, without any derivations, what is known 
...
\textbf{Pear-shaped figures and fission.} Poincar\'{e} showed in 1885 that the Jacobi sequence has in turn a point 
of bifurcation, at an axis ratio $a_{2}/a_{1}\approx0.43$, from which a sequence of \textbf{pear-shaped figures} 
