...
for a, b in zip(prem.BOUNDARIES[:-1], prem.BOUNDARIES[1:]):
    x = np.linspace(a, b, 200); rr += list(x); dd += list(prem.density(np.clip(x, a + 1e-6, b - 1e-6)) / 1e3)
fig, axes = plt.subplots(1, 2, figsize=(8.0, 4.2), sharey=True, constrained_layout=True)
axes[0].plot(dd, rr, "k", lw=1.3); axes[0].set_xlabel(r"density / g cm$^{-3}$"); axes[0].set_ylabel("radius / km"); axes[0].set_xlim(0, 14)
...
axes[0].text(13.6, prem.R_CMB + 60, "CMB", ha="right", fontsize=9, color="0.4"); axes[0].text(13.6, prem.R_ICB + 60, "ICB", ha="right", fontsize=9, color="0.4")
fig.savefig(str(FIG / "L10F2.pdf")); fig.savefig(str(FIG / "L10F2.png"), dpi=200)
