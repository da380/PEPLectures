...
gs = fig.add_gridspec(2, 7, height_ratios=[3.2, 1.3])
ax = fig.add_subplot(gs[0, :])
ax.plot(mac[:, 0], mac[:, 1], "k", lw=1.4, label="Maclaurin spheroids")
ax.plot(jac[:, 0], jac[:, 1], "tab:red", lw=1.4, label="Jacobi ellipsoids")
ax.plot(angular_momentum(bif), omega2(bif), "ko", ms=6); ax.annotate("bifurcation", (angular_momentum(bif), omega2(bif)), xytext=(-70, 10), textcoords="offset points", fontsize=9)
...
ax.plot(Lp, Op, "s", color="tab:red", ms=6); ax.annotate("pear-shaped figures bifurcate", (Lp, Op), xytext=(8, 8), textcoords="offset points", fontsize=9)
imax = np.argmax(mac[:, 1]); ax.annotate("maximum rotation rate", (mac[imax, 0], mac[imax, 1]), xytext=(10, 4), textcoords="offset points", fontsize=9)
ax.set_xlim(0, 0.75); ax.set_ylim(0, 0.5)
ax.set_xlabel(r"angular momentum $L/\sqrt{GM^{3}\bar{a}}$"); ax.set_ylabel(r"$\Omega^{2}/\pi G\rho$")
ax.legend(loc="lower right", fontsize=9, frameon=False); ax.grid(True, ls=":", lw=0.5)
# shapes
members = [("sphere", np.array([1.0, 1.0, 1.0])), (r"Maclaurin, $e=0.6$", maclaurin_axes(0.6)), ("Maclaurin at bifurcation", bif),
           (r"Jacobi, $a_2/a_1=0.6$", jacobi_axes(0.6)), ("Jacobi at the pear point", pear)]
...
for k, (name, f, ls) in enumerate([("pear-shaped (schematic)", lambda s: (1 + 0.25 * s) * (1 - 0.15 * np.exp(-((s + 0.3) / 0.35) ** 2)), "-"),
                                    ("fission? (schematic)", lambda s: (1 + 0.1 * s) * (1 - 0.6 * np.exp(-(s / 0.3) ** 2)), "--")]):
    axs = fig.add_subplot(gs[1, 5 + k]); a = pear
    outline(axs, (0, 0.9), a[0], a[1], f, ls); outline(axs, (0, -1.2), a[0], a[2], f, ls)
    axs.set_xlim(-2.1, 2.1); axs.set_ylim(-2.3, 2.3); axs.set_aspect("equal"); axs.axis("off"); axs.set_title(name, fontsize=8)
fig.savefig(str(FIG / "L10F3.pdf")); fig.savefig(str(FIG / "L10F3.png"), dpi=200)
