...
\end{equation*}
where $q=r\sin\theta/\alpha$ is the ray parameter, and $r_{t}$ satisfies $r_{t}/\alpha(r_{t})=q$. This question 
...
\hardq Consider the deformation of an isotropic elastic body due to an applied surface load, $\sigma$. The relevant linearised equations of motion are
\begin{gather*}
...
Suppose that observations, $\mathbf{u}_{i}^{\mathrm{obs}}$, of the displacement vector have been made at surface locations $\mathbf{x}_i$ for $i=1,\dots,m$. An inverse problem could then be formulated to estimate the Lamé parameters, $\lambda$ and $\mu$, as well as the load, $\sigma$. To this end, we can seek to minimise the misfit
\begin{equation*}
    J=\frac{1}{2}\sum_{i=1}^{m}\frac{1}{\varepsilon_{i}^{2}}\|\mathbf{u}(\mathbf{x}_{i})-\mathbf{u}_{i}^{\mathrm{obs}}\|^{2},
\end{equation*}
where $\varepsilon_{i}$ is the standard deviation for the $i$th observation. Adapt the approach within Lecture~20 to derive sensitivity kernels for $J$ with respect to $\lambda$, $\mu$, and $\sigma$, showing how they can be calculated through one solution of the forward problem and one of an appropriate adjoint problem.
...
an earthquake on a fault of that size. By expanding $\partial|km\rangle^{*}/\partial x'_{j}$ in a Taylor series about $\mathbf{x}_{s}$, show that 
...
\item Consider a non-rotating planet of radius $a$ with uniform density $\rho$, and let $M$ be its mass.
\begin{enumerate}
...
volume $V_{0}$ is $\int p\,\dd V_{0}s$, show that the change in the internal energy of the planet is
\begin{equation*}
...
\hardp Consider a toroidal multiplet with singlets $|km\rangle=W(r)\mathbf{C}_{lm}(\theta,\varphi)$, normalised so that
\begin{equation*}
