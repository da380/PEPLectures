...
We assume for the first part of this course that $\mathbf{T}^0(\mathbf{x}) = \mathbf{0}$ for every $\mathbf{x} \in M$,
 which is one solution, though by no means the only one, of the equilibrium equations in eq.(19) and the associated boundary conditions in eq.(20). 
...
\end{equation}
This linearised relationship is \textbf{Hooke's law}. But please note that this is neither a ``law'' of nature nor merely an
...
Because $\mathbf{C}$ is symmetric, the matrix $\mathbf{Q}^{T}\mathbf{C}\mathbf{Q}$ can be made diagonal for
 ...
 energy depends only on $\mathbf{C}$ through its eigenvalues. Using this result it is only a matter of tedious algebra 
...
\end{split}
\end{equation}
where $\lambda$ and $\mu$ are as before, while $\gamma, \xi$, and $\zeta$ are additional elastic moduli. Do not fear, 
